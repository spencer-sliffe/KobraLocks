//Frontend-web/src/components/Dashboard/NCAABBSchedule.js

import React, { useEffect, useState } from 'react';
import { fetchNCAABBGames } from '../../../../api';
import './GameSchedule.css'; // We'll define styles here

const NCAABBSchedule = () => {
    const [games, setGames] = useState([]);

    useEffect(() => {
        const getGames = async () => {
            try {
                const gameData = await fetchNCAABBGames();
                setGames(gameData);
            } catch (error) {
