...
import logging
import json
from api.models import MLBGameLinks, MLBGameProps
from concurrent.futures import ThreadPoolExecutor, as_completed

class Command(BaseCommand):
    help = 'Scrape MLB game props data from ESPNBet using stored game IDs'
    ...
    def handle(self, *args, **kwargs):
        ...
        MLBGameProps.objects.all().delete()
        self.logger.info("Cleared the MLBGameProps table in the database.")
    ...
    def scrape_game_page(self, game_id):
        event_url = f"https://espnbet.com/sport/baseball/organization/united-states/competition/mlb/event/{game_id}/section/game_props"
        ...
        for section in sections:
            self.logger.info("Processing MLB prop section")
    ...
    def handle_section(self, section, game_text, game_id):
        try:
            ...
            row_titles = section.find_all('div', class_='text-style-s-medium text-primary text-primary')
            if len(row_titles) == 0:
                decimalodds = []
                americanodds = []
                ...
                self.logger.info(f"Section: {section_title}, Decimal Odds: {decimalodds}, American Odds: {americanodds}")

                MLBGameProps.objects.create(
                    game_id=game_id,
                    game=game_text,
                ...
                for row_title in row_titles:
                    solo_rows = section.find_all('div', class_='flex p-0')
                    row = row_title.text.strip()
                    decimalodds = []
                    americanodds = []
                    for solo_row in solo_rows:
                        for odd in solo_row.find_all('button', class_='relative rounded flex h-[48px] flex-1 flex-col items-center justify-center p-1.5 text-style-xs-medium button-bet-selector-deselected'):
                            decimal_odd = odd.find('span', class_='font-medium text-selector-label-deselected')
                            decimal_odd_text = decimal_odd.text.strip() if decimal_odd else ""
                            decimalodds.append(decimal_odd_text)
                            american_odd = odd.find('span', class_='font-bold')
                ...
                # Check extracted values before saving
                        ...
                        self.logger.info(f"Prop Section saved: {section_title}")
        
        except Exception as e:
            self.logger.error(f"Error processing prop section: {str(e)}")
