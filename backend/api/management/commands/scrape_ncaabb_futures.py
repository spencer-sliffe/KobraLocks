# backend/api/management/commands/scrape_ncaabb_futures.py
...
import time
from api.models import NCAABBFutures
import logging
import json

class Command(BaseCommand):
    help = 'Scrape NCAABB futures data from ESPNBet'
    ...
    def handle(self, *args, **kwargs):
        ...
        # Clear the existing NCAABB Futures in the database
        NCAABBFutures.objects.all().delete()
        self.logger.info("Cleared the NCAABBFutures table in the database.")
        ...
        # URL to scrape
        url = "https://espnbet.com/sport/baseball/organization/united-states/competition/ncaa/section/futures"
        driver.get(url)
    ...
    def handle_future(self, future):
        try:
            ...
            # Create NCAABBFutures entry
            NCAABBFutures.objects.create(
