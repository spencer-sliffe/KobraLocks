# backend/api/management/commands/scrape_nfl_player_futures.py
...
import time
from api.models import NFLPlayerFutures
import logging
import json

class Command(BaseCommand):
    help = 'Scrape NFL player futures data from ESPNBet'
    ...
    def handle(self, *args, **kwargs):
        ...
        # Clear the existing NFL player futures in the database
        NFLPlayerFutures.objects.all().delete()
        self.logger.info("Cleared the NFLFutures table in the database.")
        ...
        # URL to scrape
        url = "https://espnbet.com/sport/football/organization/united-states/competition/nfl/section/player_futures"
        driver.get(url)
    ...
    def handle_future(self, future):
        try:
            ...
            # Create NFLFutures entry
            NFLPlayerFutures.objects.create(
