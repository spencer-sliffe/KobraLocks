# backend/api/management/commands/scrape_games.py
...
class Command(BaseCommand):
    help = 'Scrape NCAABB games data from ESPNBet'
    ...
    def handle(self, *args, **kwargs):
        ...
        # Clear the existing games in the database
        Game.objects.all().delete()
        self.logger.info("Cleared the Game table in the database.")
        ...
        # URL to scrape
        url = "https://espnbet.com/sport/baseball/organization/united-states/competition/ncaa/featured-page"
        driver.get(url)
    ...
    def handle_live_game(self, game):
        try:
            ...
            odds_buttons = game.find_all('button', class_='button-bet-selector-hidden') + game.find_all('button', class_='button-bet-selector-deselected')
            team1_spread = odds_buttons[0].find('span', {'class': 'font-medium'}).text.strip() if len(odds_buttons) > 0 else None
            team1_spread_odds = odds_buttons[0].find('span', {'class': 'font-bold'}).text.strip() if len(odds_buttons) > 0 else None
            team1_total = odds_buttons[1].find('span', {'class': 'font-medium'}).text.strip() if len(odds_buttons) > 1 else None
            ...
            team2_spread = odds_buttons[3].find('span', {'class': 'font-medium'}).text.strip() if len(odds_buttons) > 3 else None
            team2_spread_odds = odds_buttons[3].find('span', {'class': 'font-bold'}).text.strip() if len(odds_buttons) > 3 else None
            team2_total = odds_buttons[4].find('span', {'class': 'font-medium'}).text.strip() if len(odds_buttons) > 4 else None
    ...
    def handle_scheduled_game(self, game):
        try:
            time_info_button = game.find('button', class_='text-primary')
            ...
            odds_buttons = game.find_all('button', class_='button-bet-selector-deselected')
            team1_spread = odds_buttons[0].find('span', {'class': 'font-medium'}).text.strip() if len(odds_buttons) > 0 else None
            team1_spread_odds = odds_buttons[0].find('span', {'class': 'font-bold'}).text.strip() if len(odds_buttons) > 0 else None
            team1_total = odds_buttons[1].find('span', {'class': 'font-medium'}).text.strip() if len(odds_buttons) > 1 else None
            team1_total_odds = odds_buttons[1].find('span', {'class': 'font-bold'}).text.strip() if len(odds_buttons) > 1 else None
