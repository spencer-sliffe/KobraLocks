# backend/api/management/commands/scrape_ncaafb_games.py
...
import time
from api.models import NCAAFBGame
import datetime
import pytz
import logging

class Command(BaseCommand):
    help = 'Scrape NCAAFB games data from ESPNBet'
    ...
    def handle(self, *args, **kwargs):
        ...
        # Clear the existing NCAAFB games in the database
        NCAAFBGame.objects.all().delete()
        self.logger.info("Cleared the NCAAFBGame table in the database.")
    ...
    def handle_live_game(self, game):
        try:
            ...
            quarter_info = game.find_all('span', class_='flex items-center gap-1 py-0.5')
            quarter_state = quarter_info[1].text.strip() if len(quarter_info) > 1 else None  # Correctly fetch the quarter state
            ...
            team2_spread = odds_buttons[3].find('span', {'class': 'ffont-medium text-selector-label-deselected'}).text.strip() if len(odds_buttons) > 3 else None
